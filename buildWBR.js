...
const drawWelcome = () => {
  ...
  drawText("https://github.com/cedrosdev/WinnetouJs/#readme");
  drawBlankLine();
...
const drawFinal = () => {
  ...
  drawBlankLine();
  drawTextBlock(
    "To init a WinnetouJs blank project, run 'node ./node_modules/winnetoujs/build', but take care, it will overwrite your existing files. After that, run 'npm i' to install WinnetouJs dependencies. "
  );
  drawBlankLine();
  if (errorsCount > 0) {
...
console.clear();
drawWelcome();
drawText("Coping Winnetou Bundler (wbr) ...");
drawBlankLine();
